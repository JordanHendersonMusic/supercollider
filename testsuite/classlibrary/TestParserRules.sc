...
/*		var foo = 10;
		...
		var d = #[(
			...
			true : true,
			false : false,
			nil : nil, // This is currently valid in the parser, but means nothing! Consider removing this at somepoint.
			[] : [],
			() : (),
			keybin: \keybin
		)];

		this.assertEquals(d[0][1], 1);
		this.assertEquals(d[0][1.0], 1.0);
		this.assertEquals(d[0][$ ], $:);
		this.assertEquals(d[0][\b], \b);
		this.assertEquals(d[0][\foo], \bar);
		this.assertEquals(d[0][true], true);
		this.assertEquals(d[0][false], false);
		this.assertEquals(d[0][\keybin], \keybin);*/
	}
